...
class  LamportTotalOrderAlgorithm(Thread):
    ...
    def  compareAndUpdateLogicalClock(self,processlogicaltime):
    ...
    def run(self):
        ...
        try:
          for event in self.ProcessEvents:
                  print(event)
                  self.timestamp=self.timestamp+1
                  print(f"Delivery of {self.pid} : {self.pid}.{event[0]}")

                  self.timestamp=self.timestamp+1
                  msg=self.pid+","+event+","+str(self.timestamp)
                  self.process1.soc.sendall(msg.encode())

                  pmsg1=self.process1.soc.recv(1024)
                  self.respacknowledge.append(pmsg1.decode('utf-8'))
                  
                  print(f"Delivery of {self.pid} : {pmsg1.decode('utf-8')}")
                  ...
                  print(f"Delivery of {self.pid} : {pmsg2.decode('utf-8')}")          
        except Exception as e:
    ...
    def maintainBufferAndAcknowledge(self,socconn):
            try:
                 while(True):
                      ...
                      self.compareAndUpdateLogicalClock(int(msg[2]));
...
if(__name__ == "__main__"): 
   ltoa=LamportTotalOrderAlgorithm("process_1")
   x = Thread(target=mainThread, args=())
   x.start()

   p1=Process(8882,ltoa)
   p2=Process(8883,ltoa)
   while(True):
